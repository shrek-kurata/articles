...
Skillが増えてくると、「root agentのコンテキストが手順書で埋まってしまう」「危険な操作を意図せず自動発動させたくない」「subagentに必要な知識だけを渡したい」といった課題が出てきます。これらはすべて**コンテキストの分離と制御**の問題です。

Claude Codeでは、Skillsとサブエージェントの組み合わせに対して3つの制御方法が用意されています。それぞれの挙動とユースケースを整理します。
...
compaction後の復旧策だけでなく、**そもそもcompactionを発生させにくい設計**を意識することが最も効果的です。

**対策0: compactionを起こさない設計を心がける**

compactionは「コンテキストウィンドウが限界に達したとき」に発生します。つまり、コンテキスト消費を抑える設計ができれば、compaction自体を回避できます。

- **タスクを小さく分割する**: 1セッションで巨大なリファクタリングをしない。1つのPR・1つの機能単位でセッションを切り替えます
- **`context: fork` を積極的に使う**: 大量のファイル読み書きを伴うSkillは別コンテキストで実行し、main contextの消費を抑えます
- **不要なツール結果を溜めない**: 大きなファイルの全文読み込みを避け、必要な部分だけを読むようにします
- **CLAUDE.mdを短く保つ**: セクション1で述べた通り、CLAUDE.mdが長いとその分だけ毎回コンテキストを消費し、compactionまでの猶予が減ります
- **Skill本文を簡潔にする**: Skill発動時に注入される本文が長いほどコンテキストを圧迫します。手順書は要点を絞り、詳細はスクリプトや外部ファイルに委譲します
