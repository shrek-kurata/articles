...
「CLAUDE.mdが200行を超えて、何が書いてあるか自分でも把握できない」── そんな状態になっていないだろうか。

[Agent Skills](https://code.claude.com/docs/en/skills)は「専門知識のオンデマンドロード」を実現する仕組みだ。必要なときだけSKILL.md本文を読み込むことで、CLAUDE.mdの肥大化を防ぎ、コンテキストウィンドウを効率的に使える。しかし、設計を間違えるとdescriptionだけでbudgetを食い潰し、むしろコンテキストを圧迫する。

この記事では「何をSkillに切り出すか」「何個まで入れていいのか」「モノレポでどう配置するか」「compactionで何が消えるか」を**設計論**として整理する。作り方のチュートリアルではなく、実運用で直面する設計判断にフォーカスした内容だ。
...
最初に整理すべきは「何をCLAUDE.mdに残し、何をSkillに分離するか」だ。判断の軸は**頻度**と**分量**の2つ。
...
[CLAUDE.mdは毎セッション全文が読み込まれる](https://code.claude.com/docs/en/how-claude-code-works)。一方、Skillsはdescription（30〜100トークン）だけが常時ロードされ、本文はオンデマンドで読み込まれる。つまり:

- **CLAUDE.md**: 常にコンテキストを消費するので短く保つ
- **Skills**: 使わない限りdescriptionしか消費しないので、長い手順書を入れても問題ない

### `/init` の出力は削るべき

`/init` コマンドが生成するCLAUDE.mdは、リポジトリ構造を網羅的に記述するため冗長になりがちだ。生成後に以下を削ることを推奨する:

- ディレクトリ構造の全量リスト（Claudeはファイルを自分で読める）
- 各ファイルの説明文（Claudeはコードから理解できる）
- 技術スタックの詳細バージョン（package.jsonから読める）

残すべきは「Claudeが自力では判断できない、プロジェクト固有の意思決定」だけだ。

---

## 2. Agent Skillsの3段階ロード

Skillsのロードは3段階で行われる。この仕組みを理解することが設計の起点になる。
...
セッション開始時に、すべてのSkillの`name`と`description`がシステムプロンプトにロードされる。**SKILL.md本文は読まれない。** これがトークン節約の核心だ。

### Stage 2: ファイル操作時 ── ネスト発見

プロジェクトルートでセッションを開始した場合、サブディレクトリの`.claude/skills/`は**未ロード**の状態だ。そのディレクトリ内のファイルにアクセスした時点で、追加のSkillがディスカバリされる。
...
ユーザーの入力に対し、Claudeがdescriptionに基づいて「このSkillが必要だ」と判断すると、SKILL.md本文を読み込んでコンテキストに注入する。インターフェースには`Reading [skill-name]`と表示される。

### ルーティング判断の仕組み

:::message alert
以下はアーキテクチャ的に妥当な説明だが、Anthropicが公式にこの通り説明しているわけではない。技術的な理解としての記述である。
:::

Skillの発動判断は、if-elseのルールエンジンではなく、**LLMのforward pass（推論プロセス）の中で行われる**。ユーザー入力とSkill descriptionの意味的関連度がニューラルネットの推論として評価される。

だからこそ、descriptionには**ユーザーが使いそうな言葉（トリガーワード）を含める**ことが重要だ。「このSkillはPDFを生成するときに使う」なら、descriptionに`PDF`, `generate`, `レポート`, `出力` などのキーワードを盛り込む。
...
Skill descriptionの合計は、**コンテキストウィンドウの2%で動的にスケール**する。フォールバック値は**16,000文字**だ。環境変数`SLASH_COMMAND_TOOL_CHAR_BUDGET`でオーバーライドもできる。
...
この109文字はSkillのメタデータ（nameタグ、descriptionタグ等）に使われる固定コストだ（[alexey-pelykh氏による実測](https://github.com/anthropics/claude-code/issues/13099)）。

### 何個まで大丈夫か ── 具体的な計算

16,000文字のbudgetを前提に計算する:
...
**実例**: 63個のSkillをインストールした環境で、42個しか表示されず**21個（33%）が隠れた**事例が報告されている（[GitHub Issue #13099](https://github.com/anthropics/claude-code/issues/13099)）。
...
プロジェクト内の`.claude/skills/`ディレクトリは、**ネストされたサブディレクトリでも自動ディスカバリされる**。これはCHANGELOGで「Added automatic discovery of skills from nested .claude/skills directories」として追加された機能だ。

参照: [Skills公式ドキュメント](https://code.claude.com/docs/en/skills)

### トリガータイミング

ネスト発見が発動するのは、**そのサブディレクトリ内のファイルにアクセスしたとき**だ。
...
この設計はモノレポと相性が良い。各パッケージチームが自分のSkillsを管理でき、他パッケージのSkillsがbudgetを消費しない。

### `~/.claude/skills/` の制限事項

:::message alert
`~/.claude/skills/`（ユーザースキル）は**再帰スキャンされない**。フラット構造が必須であり、サブディレクトリにSKILL.mdを配置しても発見されない。この制限は[GitHub Issues #18192](https://github.com/anthropics/claude-code/issues/18192), [#16438](https://github.com/anthropics/claude-code/issues/16438), [#20805](https://github.com/anthropics/claude-code/issues/20805)で複数回Feature Requestされているが、2026年3月時点で未実装だ。

回避策として、シンボリックリンクでフラット化する方法がある:
:::
...
同名のSkillが複数の階層に存在する場合、以下の優先順位で解決される（[公式ドキュメント](https://code.claude.com/docs/en/skills)）:
...
- **プラグイン**はプレフィックス付き（`plugin-name:skill-name`）なので実質衝突しない
- **ネスト発見**で追加されたSkillは、上位ディレクトリのSkillと名前が衝突する場合、上位が優先される
...
Skillsとサブエージェントの組み合わせには3つの制御方法がある。それぞれの挙動とユースケースを整理する。
...
[公式ドキュメント](https://code.claude.com/docs/en/sub-agents)によると、`context: fork`を指定したSkillは**別のコンテキストウィンドウ**で実行される。main agentのコンテキストには要約のみが返るため、main contextを消費しない。
...
:::message alert
[GitHub Issue #17283](https://github.com/anthropics/claude-code/issues/17283)で「Skillツール経由で呼ぶとforkが無視される」バグが報告されている。使用するClaude Codeのバージョンで動作確認を推奨する。
:::

### subagentの `skills` フィールド

[subagent公式ドキュメント](https://code.claude.com/docs/en/sub-agents)に明記されている重要な仕様がある:

> The full content of each skill is injected into the sub-agent's context, not just made available for invocation

つまり、subagentの`skills`フィールドに指定したSkillは**遅延ロード（description → 本文）ではなく、全文が起動時からsubagentに注入**される。また、subagentは親の会話からSkillsを継承しない。明示的に`skills:`で列挙する必要がある。

### `disable-model-invocation: true` の効果

[公式ドキュメント](https://code.claude.com/docs/en/skills)によると、このフラグを付けたSkillは、Claudeが自動発動しない。ユーザーの`/skill-name`コマンドでのみ起動する。

実質的に「descriptionを自動ルーティングの対象から外す」スイッチとして機能する。
...
[MCP公式ドキュメント](https://code.claude.com/docs/en/mcp)および[Tool Search Tool](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool)によると、MCPツールには`defer_loading`による遅延ロードの仕組みがある。
...
Skillsには`defer_loading`は**適用されない**。これは明確に別レイヤーとして設計されている（[alexey-pelykh氏の分析](https://github.com/anthropics/claude-code/issues/13099)）。
...
ただし、プラグインエコシステムの拡大により、Skillが100個を超える環境が現れれば、同様の仕組みが必要になる可能性はある。現時点では、description budgetの最適化（セクション3参照）で対処可能だ。
...
長時間のセッションで避けて通れないのが**compaction**だ。コンテキストウィンドウが限界に達すると、Claude Codeは会話を要約して圧縮する。このとき、**SKILL.md本文の手順は消失する**。
...
- **[Issue #13919](https://github.com/anthropics/claude-code/issues/13919)**: compaction後にSkillの手順を完全に忘れ、同じタスクを最初からやり直す事例が詳細に報告されている
- **[Issue #20466](https://github.com/anthropics/claude-code/issues/20466)**: 逆にcompaction後にSkillを再実行してしまう（終わったタスクをもう一度やる）バグも報告されている
...
CLAUDE.mdはcompaction後もディスクから再読み込みされるため、この指示は保持される。

**対策2: 手動 `/compact` でフォーカスを指定する**

自動compactionに任せるのではなく、手動で`/compact`を実行し、何を残すかを意図的にコントロールする。重要な作業コンテキストがあるときは、compaction前に要点をCLAUDE.mdやメモファイルに書き出す運用も有効だ。

**対策3: `context: fork` で長い作業を分離する**

Skill実行を`context: fork`で別コンテキストに分離すれば、main contextの圧迫を防ぎ、compactionの頻度そのものを下げられる。
...
Agent Skillsは2025年12月にAnthropicが**オープンスタンダード**としてリリースした（[agentskills.io](https://agentskills.io)）。同じSKILL.md形式が以下のツールでもサポートされている:
...
ただし、以下のfrontmatterフィールドはClaude Code固有の拡張であり、他ツールではサポートされない場合がある:
...
**ポータブルにしたい場合は、frontmatterを`name`と`description`のみに留めるのが安全だ。**

### マルチエージェント共有パターン

複数のエージェントツール間でSkillsを共有する場合、シンボリックリンクが有効だ（[atama plus方式](https://zenn.dev/atamaplus/articles/6d8c3615ff3f33)）:
...
この方法なら、Skillの更新を1箇所で行えば全ツールに反映される。ただしClaude Code固有のfrontmatter（`context: fork`等）はportableなSKILL.mdには含めず、Claude Code専用のSkillとして別途管理することを推奨する。
...
`/context`を実行すると、コンテキストウィンドウ内の各要素のトークン消費量が一覧表示される。Skillsのbudget超過が発生している場合は、以下のような警告が表示される:
...
`Showing 42 of 63 skills due to token limits`という警告が出たら、descriptionの文字数を削減するか、不要なSkillを削除する必要がある。
...
トランスクリプトモード（`Ctrl+O`）を有効にすると、ClaudeがSkillを発動する過程をリアルタイムで確認できる。`Reading [skill-name]`のログが表示されれば、SKILL.md本文が読み込まれている証拠だ。発動の有無が不明な場合はここで確認する。

---

## 10. 設計チェックリスト

記事の内容を実践的なチェックリストにまとめた。新しいSkillを追加するとき、チーム導入時、トラブル発生時に使ってほしい。
